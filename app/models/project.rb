  ...
  def self.allowed_to_condition(user, permission, options={})
    ...
    if user.admin?
      ...
      if user.logged?
        ...
        project_ids = user.allowed_in_projects(permission).all.map(&:project_id).compact
        statement_by_role[:blubs] = "#{Project.table_name}.id IN (#{project_ids.join(',')})" if project_ids.any?
      else
